...
namespace HAPISPACE {
	...
	*/
	...
	public:
		...
		CHapiXMLAttribute(const std::string &name, const char cvalue[]) : m_name(name), m_stringValue(cvalue) {}
		template <typename T>		
		CHapiXMLAttribute(const std::string &name, const T &value) : m_name(name), m_stringValue(std::to_string(value)) {}
